...
I found data from [bts.gov](https://www.bts.gov/) and [faa.gov](https://www.faa.gov/airports) available for download. 

My goal with this data was to create columns I can merge together for my own data table. 

## Process

### The first file I read was called ranking.xlsx and I previewed it.

**rankings** 
- U.S. Airports, ranked by 2022 Systemwide* Scheduled Enplanements Passenger numbers in millions (000,000)
- [Ranking source](https://www.bts.gov/topics/airlines-and-airports/airport-rankings-2022)
...
### Next, I needed to add IATA codes for each airport. This will allow me to use API later to fetch more information for the data table.
...
### Then, I used data from rankings, originatingPassengers, & airportPassengersBoarded in a for-loop to create lists that will be used as columns.
...
### At this point, the rankedLOCID was still missing values for some of the IATA codes, so I made a request from chatGPT to create a file with the known airport name to populate IATA codes.
...
### Next, I combined rank, cleanedRankedLOCID (IATA vals), and numPassengers into a DataFrame.
...
### After, I wanted to add more columns, so I used the DataFrame: originatingPassengers.
...
### Next, I added the new column to my DataFrame.
...
Now that my table has 4 solid columnns, *2022 Rank*,	*Airport*, *2022 Enplaned Passengers*, & *2022 Originating Domestic Passengers*, I knew I wanted more information and decided to use API to request ICAO codes.
...
### Then, I used a secondary API to populate Longitude, Lattitude, Elevation, and State columns.
...
### After I created the airportDF from the API request, I merged on the new columns I wanted to my exisiting DataFrame.
...
### Next, I cleaned up my main DataFrame and renamed the columns.
...
### I was happy with the DataFrame, but I wanted to add more interesting information about each airport.
After searching online for available data, I found this website: [https://www.stratosjets.com/blog/us-airport-rankings/](https://www.stratosjets.com/blog/us-airport-rankings/) with an updated 2022 table of US airports and for each...  
- Flights Per Year	
- Delays %	
- Canceled %	
- Avg. Delay (Mins)	
...
### Then, I did some cleaning and organization to the finalDF DataFame
...
**Note:** I use the variable *partialpath* throughout to refer to the folder location with my stored data sets. All of this can be viewed at: [Github Repo](https://github.com/lucystorts/386Project)
